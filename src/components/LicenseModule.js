...
export default function LicenseModule(p) {
  ...
  const [Users, setUsers] = useState([
    ...
    // { code: "IPICLLOG08", name: "IBL Logistic Staff - DMPI",      id: "84"    },
  ...
  const [allocatedLicenseRequested, setallocatedLicenseRequested] =
    useState(false);
  const AssignUserTo = async (e) => {
    setallocatedLicenseRequested(false);
    setloading(true);
    ...
    setloading(false);
    setallocatedLicenseRequested(true);
  };
  const GetAssignUserTo = async (e) => {
    // console.log(Users[i]);
    await EXEC_SQL_InsertOne(920, setUsers, p.userid);
  };
  useEffect(() => {
    GetAssignUserTo();
  }, [p.userid]);
  ...
  return (
    <>
      <div className="">
        {allocatedLicenseRequested ? (
          <>
            <div className="flex mx-auto w-fit">
              <ThumbUpIcon className="w-5 h-5 mt-2.5 mr-2" />
              <p>
                License Requested <br></br> Check your SAP B1 Application
              </p>
            </div>
          </>
        ) : (
        ...
        {Users.map((item, index) => (
          <div className=" flex frame">
            ...
            <div className="mr-4">{item.licensCode}</div>
